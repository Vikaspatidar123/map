...
import { IcMFairport } from '@cogoport/icons-react';
...
function AirList({ listFilters, setListFilters }) {
		...
		<div className={styles.container}>
			{loading ? (
				...
				: data.map((item) => {
					...
					const size = (item?.air_route_link || []).length;
					const air_route_link = !isActive
						? (item?.air_route_link || []).filter((loc, i) => !i || i === size - 1)
						: (item?.air_route_link || []);
							...
							<div className={styles.header}>
								<div>
									...
									{item?.operator?.business_name && (
										...
										>
											<IcMFairport className={styles.vessel_icon} />
											{item?.operator?.business_name}
										</p>
									)}
